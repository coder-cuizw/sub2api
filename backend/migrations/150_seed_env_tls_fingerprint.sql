...
-- 内置 TLS 指纹模板：「Claude Code Linux x64 (Node.js v22.22.2)」。
-- 采集自 sub2api 实际运行环境（Linux x64 / Node.js v22.22.2，OpenSSL 3.5.5），
-- 作为 Claude 伪装可直接选用的内置指纹之一，无需手动从采集站粘贴 YAML。
--
-- 真实性：已用真实 @anthropic-ai/sdk（Claude Code 实际使用的 fetch/undici HTTP 栈）
--   抓取其发往 api.anthropic.com 的 ClientHello，与本模板逐字段一致——SDK 未自定义
--   TLS，直接沿用本环境 OpenSSL 3.5.5 的默认 ClientHello。
--
...
VALUES (
    'Claude Code Linux x64 (Node.js v22.22.2)',
    '真实 Claude Code 客户端指纹，经 @anthropic-ai/sdk 实测验证。采集自 sub2api 运行环境（Linux x64 / Node.js v22.22.2，OpenSSL 3.5.5）。含后量子组 X25519MLKEM768。',
    false,
